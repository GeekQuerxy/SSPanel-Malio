            ...
            <section class="content-inner margin-top-no">
                ...
                <div class="card">
                    <div class="card-main">
                        ...
                        {if strpos($edit_config->key,'.enable.')===false}
                            <div class="form-group form-group-label">
                                <label class="floating-label" for="value">配置值</label>
                                <textarea class="form-control maxwidth-edit" id="value" rows="5">{$edit_config->value}</textarea>
                            </div>
                        {else}
                            <div class="form-group form-group-label">
                                <label for="value">
                                    ...
                                    <select id="value" class="form-control maxwidth-edit" name="value">
                                        <option value="0" {if $edit_config->value=='0'}selected{/if}>关闭</option>
                                        <option value="1" {if $edit_config->value=='1'}selected{/if}>开启</option>
                                    </select>
                                </label>
                            </div>
                        {/if}
...
<script>
    window.addEventListener('load', () => {
        function submit() {
            $.ajax({
                type: "PUT",
                url: "/admin/config/telegram/{$edit_config->key}",
                dataType: "json",
                data: {
