...
function checkin() {
  ...
  $.ajax({
    ...
    success: function (data) {
      $('#checkin-div').html('<a href="#" class="btn btn-icon disabled icon-left btn-primary"><i class="far fa-edit"></i> 已签到</a>')
  ...
  ! function ($) {
    ...
    ChartC3.prototype.init = function () {
        c3.generate({
          ...
          data: {
            columns: [
              ['已用 ' + lastusedTraffic, last100],
              ['今日 ' + todayusedTraffic, today100],
              ['可用 ' + unusedTraffic, unuse100],
            ],
            type: 'donut'
          },
          color: {
      ...
      success: function (data) {
        if (data.ret) {
          ...
          swal({
            ...
            title: '购买失败',
            text: data.msg,
            footer: '<a href="/user/code">前往我的钱包充值</a>'
          })
        }
      },
      error: function (jqXHR) {
...
function oneclickImport(client, subLink) {
  ...
  if (!isMobie() && client != 'clash' & client != 'ssr' ) {
    Swal.fire({
      title: '提示',
      html: '导入配置到 <span style="text-transform: capitalize;">' + client + '</span> 需要在手机浏览器操作，无法在电脑端导入。',
      type: 'warning',
      showCancelButton: true,
      ...
      success: function (data) {
        if (data.ret) {
          ...
          if (data.type == 'url') {
            ...
            var redirect_url = '/user/qrcode?type='+type+'&qrcode='+url+'&tradeno='+data.tradeno+'&price='+price;
            if (user_location == 'code') {
              window.location.href = redirect_url; //  qrcode to url
            } else {
              $('#maliopay-modal').modal({
                backdrop: 'static',
                keyboard: false
              ...
              $('#maliopay-modal').modal('show');
              $('#to-maliopay').attr("href", redirect_url); // qrcode to url
            }
          }
        } else {
          Swal.fire(
    ...
    success: function (data) {
      if (data.ret) {
        ...
        if (need_to_pay_price <= 0) {
          ...
          topUp(need_to_pay_price, payment);
        }
